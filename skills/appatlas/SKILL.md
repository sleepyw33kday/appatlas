...
2. **Crawl (one Sonnet agent per section, sequential)** — each agent reads the briefing, owns one section, saves screenshots + writes `spec.md`, reports a 5–10 line summary. Update the README row as each returns; dispatch the next.
...
4. **Index** — build the Mobbin-style self-contained HTML: collapsible app-hierarchy tree (nav group → page → sub-tab → modal) that filters a screenshot-card gallery with search + lightbox, per-section specs embedded. Compress thumbs (~700px JPEG q50 → data URIs). Publish as an Artifact (private by default) when the user asked for a browsable index; smoke-test locally first. Builder + pitfalls in the playbook.
